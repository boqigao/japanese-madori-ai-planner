...
- **THEN** generator snaps to 8190×9100mm (nearest 910mm multiples) and reports the snapped values

#### Scenario: Already-aligned envelope
- **WHEN** user provides `--envelope 9.1x6.37`
- **THEN** generator produces 9100×6370mm (exact 910mm multiples, no adjustment needed)
...
- **THEN** generator creates 1 washroom (fixed 1820×1820mm) and 1 bath (fixed 1820×1820mm) with required adjacency between them

#### Scenario: Compact wet shorthand
- **WHEN** a room spec is `ws+shower`
- **THEN** generator creates 1 washstand (fixed 910×910mm) and 1 shower (fixed 910×1365mm) with required adjacency between them
...
- **WHEN** envelope is 8190×9100mm and F1 has rooms ldk, bed, hall, entry, storage, closet with no user `@target` overrides
...
- **WHEN** envelope is 10010×11830mm (10×12m snapped) with 5LDK
...
- **WHEN** envelope is 7280×7280mm (7×7m snapped) with 5LDK and rooms hit min bounds
...
#### Scenario: 5LDK distribution on 8×9 envelope
