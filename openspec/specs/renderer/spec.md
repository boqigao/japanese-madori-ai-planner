...
The system MUST draw each space as a labeled rectangle with distinct fill color per room type.

#### Scenario: Room coloring
- GIVEN a floor with spaces (LDK, bedroom, bath, etc.)
- WHEN the renderer draws the floor
- THEN each space is drawn as a filled rectangle using the color from `SPACE_COLORS`
...
### Requirement: Legend
The system MUST include a legend mapping room type colors to names, plus door and window symbols.

#### Scenario: Legend display
- GIVEN a floor plan with multiple room types
- WHEN the renderer draws the floor
- THEN a legend box is drawn showing color swatches for used room types, plus door and window symbol explanations
...
The system MUST draw a per-floor and total area summary box showing sqm and tsubo.

#### Scenario: Area summary
- GIVEN a 2-floor plan
- WHEN the renderer draws the summary
- THEN a box shows each floor's area in sqm/tsubo and the total

### Requirement: Door Symbols
The system MUST draw door symbols (wall cut + swing arc) between adjacent spaces where appropriate.

#### Scenario: Interior door
- GIVEN two adjacent spaces (e.g., hall and bedroom)
- WHEN the renderer draws the topology edge
- THEN a door symbol with a wall opening and swing arc is rendered on their shared edge
